...
export const metadata: Metadata = {
  title: 'Support — Tenkhee Plus',
  description:
    'Tenkhee Plus customer support. Get help with your account, workouts, subscriptions, shop orders, and technical issues.',
...
const sections = [
  {
    ...
    body: [
      'For questions, technical issues, billing help, or feedback about the Tenkhee Plus mobile app, email our support team. Please include your registered email address and a brief description of the issue so we can assist you faster.',
      'We typically respond within 1–3 business days. Urgent account or payment issues are prioritized.',
    ],
    bodyMn: [
      'Tenkhee Plus апптай холбоотой асуулт, техникийн асуудал, төлбөр, санал хүсэлтээр дэмжлэгийн и-мэйлээр холбогдоно уу. Бүртгэлтэй и-мэйл хаяг болон асуудлын товч тайлбарыг заавал бичнэ үү.',
  ...
  },
  {
    ...
    body: [
      'Tenkhee Plus offers premium membership plans within the app. Payment may be processed via QPay or bank transfer, depending on current settings.',
    ...
    bodyMn: [
      'Tenkhee Plus апп дотор premium гишүүнчлэлийн багц санал болгодог. Төлбөр QPay эсвэл банкны шилжүүлгээр хийгдэж болно.',
  ...
  },
  {
    ...
    body: [
      'Workout plans, progress tracking, and educational content are updated through the Tenkhee Plus admin system. If content appears missing or incorrect, let us know which screen and exercise you were viewing.',
...
export default function SupportPage() {
  ...
  return (
    <LegalPageShell title="Support" subtitle={`Дэмжлэг · Last updated: ${lastUpdated}`}>
      <p className="mb-8 rounded-lg border border-white/10 bg-white/[0.03] p-4 text-sm leading-relaxed text-white/60">
        This page is the official support URL for the Tenkhee Plus mobile app, submitted to Google
        Play and Apple App Store. It is available publicly at{' '}
        <strong className="text-white">/support</strong>.
      </p>

      <section className="mb-10 rounded-xl border border-white/10 bg-white/[0.03] p-6">
        <h2 className="text-lg font-bold text-white">Contact</h2>
        <h3 className="mt-1 text-base font-semibold text-[#ff453a]">Холбоо барих</h3>
        <dl className="mt-5 space-y-4">
          {contactItems.map((item) => (
            <div key={item.label} className="grid gap-1 sm:grid-cols-[180px_1fr]">
              <dt className="text-sm font-medium text-white/55">
                {item.label}
                <span className="mt-0.5 block text-xs text-[#ff453a]/80">{item.labelMn}</span>
              </dt>
              <dd className="text-sm text-white">
                {item.href ? (
                  <a href={item.href} className="font-medium text-[#ff453a] hover:underline">
                    {item.value}
                  </a>
                ) : (
                  item.value
      ...
      <div className="mt-16 border-t border-white/10 pt-8 text-center text-sm text-white/50">
        <p className="space-x-4">
          <a href="mailto:support@tenkhee.mn" className="text-[#ff453a] hover:underline">
            support@tenkhee.mn
          </a>
          <Link href="/privacy" className="text-[#ff453a] hover:underline">
            Privacy Policy
          </Link>
        </p>
      </div>
